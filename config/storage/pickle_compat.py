...
from .json_storage import derive_key, save_config_json, create_new_config
...
def convert_to_v2_format(
    ...
    if 'paData' in legacy_config:
        ...
        if 'paInfraSubnet' in pa_data:
            v2_config['infrastructure']['shared_infrastructure_settings']['infrastructure_subnet'] = pa_data['paInfraSubnet']
        
        # BGP AS
        if 'paInfraBGPAS' in pa_data:
            v2_config['infrastructure']['shared_infrastructure_settings']['infra_bgp_as'] = pa_data['paInfraBGPAS']
        ...
        if 'paPortalHostname' in pa_data:
            v2_config['infrastructure']['mobile_agent']['portal_name'] = pa_data['paPortalHostname']
    ...
    # Add migration note
    v2_config['metadata']['migration_note'] = f"Migrated from legacy format on {datetime.utcnow().isoformat()}Z"
