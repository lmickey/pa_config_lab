"""
Policy and rule model classes for Prisma Access folder/snippet configuration.

This module contains ConfigItem subclasses for policy/rule types available
in Prisma Access folder and snippet configuration:
- Security rules (firewall policies)
- Decryption rules (SSL/TLS decryption policies)
- Authentication rules
- QoS policy rules

Note: NAT and PBF rules are only available in on-prem firewall (ngfw-shared)
configuration and are not part of Prisma Access folder/snippet workflows.
NAT configuration may be included as properties within ServiceConnection
