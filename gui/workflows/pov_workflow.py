...
class POVWorkflowWidget(QWidget):
    ...
    def _create_panorama_setup_tab(self):
        ...
        fw_user_row = QHBoxLayout()
        self.infra_fw_user = QLineEdit()
        self.infra_fw_user.setText("admin")
        fw_user_row.addWidget(self.infra_fw_user)
        infra_fw_user_copy = QPushButton("Copy")
        infra_fw_user_copy.setFixedWidth(50)
        infra_fw_user_copy.setStyleSheet("QPushButton { padding: 4px; }")
        infra_fw_user_copy.clicked.connect(lambda: self._copy_to_clipboard(self.infra_fw_user.text(), "Username"))
        fw_user_row.addWidget(infra_fw_user_copy)
        fw_conn_layout.addRow("Username:", fw_user_row)

        fw_pass_row = QHBoxLayout()
        ...
        fw_pass_row.addWidget(self.infra_fw_pass)
        infra_fw_show_btn = QPushButton("Show")
        infra_fw_show_btn.setFixedWidth(50)
        infra_fw_show_btn.setCheckable(True)
        infra_fw_show_btn.setStyleSheet("QPushButton { padding: 4px; }")
        infra_fw_show_btn.clicked.connect(lambda checked: self.infra_fw_pass.setEchoMode(
            QLineEdit.EchoMode.Normal if checked else QLineEdit.EchoMode.Password
        ))
        fw_pass_row.addWidget(infra_fw_show_btn)
        infra_fw_pass_copy = QPushButton("Copy")
        infra_fw_pass_copy.setFixedWidth(50)
        infra_fw_pass_copy.setStyleSheet("QPushButton { padding: 4px; }")
        infra_fw_pass_copy.clicked.connect(lambda: self._copy_to_clipboard(self.infra_fw_pass.text(), "Password"))
        fw_pass_row.addWidget(infra_fw_pass_copy)
        fw_conn_layout.addRow("Password:", fw_pass_row)
        ...
        pano_user_row = QHBoxLayout()
        self.pano_setup_user = QLineEdit()
        self.pano_setup_user.setText("admin")
        pano_user_row.addWidget(self.pano_setup_user)
        pano_user_copy = QPushButton("Copy")
        pano_user_copy.setFixedWidth(50)
        pano_user_copy.setStyleSheet("QPushButton { padding: 4px; }")
        pano_user_copy.clicked.connect(lambda: self._copy_to_clipboard(self.pano_setup_user.text(), "Username"))
        pano_user_row.addWidget(pano_user_copy)
        pano_conn_layout.addRow("Username:", pano_user_row)

        pano_pass_row = QHBoxLayout()
        ...
        pano_pass_row.addWidget(self.pano_setup_pass)
        pano_show_btn = QPushButton("Show")
        pano_show_btn.setFixedWidth(50)
        pano_show_btn.setCheckable(True)
        pano_show_btn.setStyleSheet("QPushButton { padding: 4px; }")
        pano_show_btn.clicked.connect(lambda checked: self.pano_setup_pass.setEchoMode(
            QLineEdit.EchoMode.Normal if checked else QLineEdit.EchoMode.Password
        ))
        pano_pass_row.addWidget(pano_show_btn)
        pano_pass_copy = QPushButton("Copy")
        pano_pass_copy.setFixedWidth(50)
        pano_pass_copy.setStyleSheet("QPushButton { padding: 4px; }")
        pano_pass_copy.clicked.connect(lambda: self._copy_to_clipboard(self.pano_setup_pass.text(), "Password"))
        pano_pass_row.addWidget(pano_pass_copy)
        pano_conn_layout.addRow("Password:", pano_pass_row)
...
# Untrust NSG - permissive (firewall handles security policy)
# Required for Azure Standard SKU public IPs to allow inbound traffic
resource "azurerm_network_security_group" "untrust" {{
  name                = "{customer}-{location}-untrust-nsg"
  location            = azurerm_resource_group.pov.location
  resource_group_name = azurerm_resource_group.pov.name
  tags                = azurerm_resource_group.pov.tags
}}

resource "azurerm_network_security_rule" "untrust_allow_all_inbound" {{
  ...
  destination_address_prefix  = "*"
  resource_group_name         = azurerm_resource_group.pov.name
  network_security_group_name = azurerm_network_security_group.untrust.name
}}
...
resource "azurerm_linux_virtual_machine" "ion_{ion_name}" {{
  ...
  # ION has no Azure guest agent — short timeout avoids 20min wait for
  # Azure's provisioning check.  Auto-import recovers the VM into state.
  timeouts {{
    create = "5m"
  }}
        ...
        class FirewallInfraWorker(QThread):
            ...
            def run(self):
                try:
                    ...
                    client.create_zone(name="trust", interfaces=["ethernet1/2"])

                    self.progress.emit("Creating untrust zone...", 40)
                    client.create_zone(name="untrust", interfaces=["ethernet1/1"])
