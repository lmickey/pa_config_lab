...
class SnippetCapture:
    ...
    def get_snippet_details(self, snippet_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not snippet_data:
                ...
                print(f"    API URL: {self.api_client._make_request.__self__.__class__.__module__}")
            ...
            if not isinstance(snippet_data, dict):
                print(f"  ⚠ WARNING: Unexpected response type for snippet ID {snippet_id}")
    ...
    def capture_snippet_configuration(self, snippet_id: str, snippet_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if not snippet_info:
            ...
            print(f"  ✗ FAILED: Could not retrieve snippet details for {display_name} (ID: {snippet_id})")
            print(f"    API URL: https://api.strata.paloaltonetworks.com/config/setup/v1/snippets/{snippet_id}")
    ...
    def _normalize_snippet(self, snippet_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        normalized = {
            ...
            'path': snippet_data.get('path', f"/config/security-policy/snippets/{snippet_data.get('name', '')}"),
            'description': snippet_data.get('description', ''),
            'is_default': self._is_default_snippet(snippet_data.get('name', '')),
            'folders': snippet_data.get('folders', []),  # List of folder objects with id and name
            'shared_in': snippet_data.get('shared_in', ''),
            'last_update': snippet_data.get('last_update', ''),
            'created_in': snippet_data.get('created_in', ''),
            'metadata': {
                'created': snippet_data.get('created_in', snippet_data.get('created', '')),
                'updated': snippet_data.get('last_update', snippet_data.get('updated', '')),
    ...
    def _is_default_snippet(self, snippet_name: str) -> bool:
        ...
        return snippet_name in default_names or 'default' in snippet_name.lower()
